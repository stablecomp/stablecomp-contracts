...
import "../interface/ICakePool.sol";
...
contract FarmBooster is Ownable {
    ...
    mapping(address => IterableMapping.ItMap) public userInfo;
    ...
    function onCakePoolUpdate(
        ...
        IterableMapping.ItMap storage itmap = userInfo[proxy];
        uint256 avgDuration;
        bool flag;
    ...
    function activedPools(address _user) external view returns (uint256[] memory pools) {
        IterableMapping.ItMap storage itmap = userInfo[proxyContract[_user]];
    ...
    function _updateBoostMultiplier(
        ...
        IterableMapping.ItMap storage itmap = userInfo[_proxy];
