...
/*
    ...
    * Changed to purchase bveCVX via Curve Factory Pool
    ...
    - Remove keeper
*/
contract SCompStrategyTricryptov2 is
BaseStrategy,
CurveSwapper,
UniswapSwapper,
TokenSwapPathRegistry
{
    ...
    // ===== Convex Registry =====
    ICrvDepositor public constant crvDepositor =
    ICrvDepositor(0x8014595F2AB54cD7c604B00E9fb932176fDc86Ae); // Convert CRV -> cvxCRV
    ...
    function getName() external pure override returns (string memory) {
        return "StrategySCompTricryptov2";
    }
    ...
    function harvest() external whenNotPaused returns (uint256) {
        ...
        if (harvestData.cvxCrvHarvested > 0) {
            ...
            uint crvToSell = harvestData.cvxCrvHarvested;
            // NOTE: Assuming any CRV accumulted is only from the above swap
            crvBalance = crvToken.balanceOf(address(this));
        ...
        if (harvestData.cvxHarvested > 0) {
            uint cvxToSell = harvestData.cvxHarvested;
            _swapExactTokensForTokens(
                sushiswap,
                cvx,
                cvxToSell,
