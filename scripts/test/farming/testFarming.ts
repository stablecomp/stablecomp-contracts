...
async function deployContract(): Promise<void> {
    ...
    sCompToken = await deployScompTask.deploySCompToken();
...
async function fundAccount(account: SignerWithAddress, amountToDeposit: any, amountToLock: any): Promise<void> {
    ...
    if (amountToLock > 0) {
        let tx = await sCompToken.connect(deployer).transfer(account.address, amountToLock);
        await tx.wait();
    }
...
async function withdraw(account: SignerWithAddress, pid: any, amountToWithdraw: any): Promise<void> {
    ...
    let balanceOfAccountBefore = await sCompToken.balanceOf(account.address);

    let tx = await masterchefScomp.connect(account).withdraw(pid, amountToWithdraw);
    await tx.wait();

    let balanceOfAccount = await sCompToken.balanceOf(account.address);
...
async function createLock(account: SignerWithAddress, amountToLock: any, lockTime: any): Promise<void> {
    ...
    if (amountToLock > 0 ) {
        let txApprove = await sCompToken.connect(account).approve(veScomp.address, ethers.constants.MaxUint256);
...
async function reachInitialBlock(): Promise<void> {
    ...
    let initialBlock = await masterchefScomp.startBlock();
...
async function read(account: SignerWithAddress[]): Promise<void> {
    for (let i = 0; i < account.length; i++) {
        let currentVotingPower = await veScomp.balanceOf(account[i].address);
...
async function unlock(account: SignerWithAddress): Promise<void> {
    ...
    let balanceOfToken = await sCompToken.balanceOf(account.address);
...
async function increaseLockAmount(account: SignerWithAddress): Promise<void> {
    ...
    let txApprove1 = await sCompToken.connect(account).approve(veScomp.address, 0);
...
async function simulateCalcFarming(): Promise<void> {
    ...
    let votingBalance1 = await veScomp.balanceOf(accountDepositAddress1);
    let votingBalance2 = await veScomp.balanceOf(accountDepositAddress2);
    let votingBalance3 = await veScomp.balanceOf(accountDepositAddress3);
    let votingBalance4 = 0;
    let votingTotal = await veScomp.totalSupply();
