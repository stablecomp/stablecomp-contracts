...
// token address
let universalRouterAddress = uniswapAddress.universalRouter;
...
async function transferWithRouter(): Promise<void> {
    let amountIn = ethers.utils.parseEther("1")
    let balanceCrvBefore = await daiContract.balanceOf(deployer.address);
    let txApprove = await daiContract.connect(deployer).approve(uniswapAddress.permit2, amountIn)
    await txApprove.wait();
...
async function executeSwap(): Promise<void> {
    ...
    let txApprove = await crvContract.connect(deployer).approve(uniswapAddress.permit2, ethers.constants.MaxUint256)
    let txApprove2 = await tetherUsdContract.connect(deployer).approve(uniswapAddress.permit2, ethers.constants.MaxUint256)

    await txApprove.wait();
    await txApprove2.wait();

    let path = ethers.utils.solidityPack(["address", "uint24", "address"], [tokenAddress.crv, 10000, tokenAddress.tetherUsd])
    ...
    console.log("Balance tether before swap: ", ethers.utils.formatUnits(balanceTetherBefore, tokenDecimals.tetherUsd),
        " \nbalance after swap: ", ethers.utils.formatUnits(balanceTetherAfter,tokenDecimals.tetherUsd),
        " \ndiff: ", ethers.utils.formatUnits(diffTether, tokenDecimals.tetherUsd))
}
