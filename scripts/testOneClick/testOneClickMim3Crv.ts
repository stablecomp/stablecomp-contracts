...
let depositAccount4 : SignerWithAddress;

// contract deploy
let sCompVault : Contract;
let sCompTokenContract : Contract;
let oneClickContract : Contract;
...
let accountDepositAddress4 = "0x075e72a5eDf65F0A5f44699c7654C1a76941Ddc8"; // account have amount of token deposit
...
async function deployOneClick(): Promise<void> {
    ...
    oneClickContract = await OneClickFactory.deploy(
        uniswapV2Address,
        UniswapFactory,
        wethAddress,
        20,
    ...
    await oneClickContract.deployed();
    console.log("Contract one click deployed to:", oneClickContract.address);
...
async function impersonateAccount(): Promise<void> {
    ...
    depositAccount4 = await ethers.getSigner(accountDepositAddress4);
}
...
async function setupUtilityContract(abiCurveSwap: any): Promise<void> {
    ...
    tokenInContract = await new ethers.Contract(daiAddress, wethABI, ethers.provider);
...
async function depositWithOneClick(account: SignerWithAddress, index: any): Promise<void> {

    depositv1Value[index] = ethers.utils.parseEther("100")

    console.log("deposit dai of account is : ", ethers.utils.formatEther(depositv1Value[index]))

    // approve dai
    let txApprove = await tokenInContract.connect(account).approve(oneClickContract.address, ethers.constants.MaxUint256);
    await txApprove.wait();

    // deposit with one click - one coin
    let poolTokens = [tokenCompoundAddress, threeCrvAddress]
    let route = [[daiAddress, wethAddress, tokenCompoundAddress]]
    let tx = await oneClickContract.connect(account).OneClickIn(route, curveSwapAddress, wantAddress, poolTokens, sCompVault.address, depositv1Value[index], true, 0, 100)
    await tx.wait();
...
main()
    .then(async () => {
        ...
        await depositWithOneClick(depositAccount4, 3);
