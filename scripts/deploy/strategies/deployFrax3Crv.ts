...
async function setupContract(): Promise<void> {
    ...
    await sCompStrategy.connect(deployer).setTokenSwapPathV2(tokenAddress.crv, tokenAddress.frax, [tokenAddress.crv, tokenAddress.weth, tokenAddress.frax],0);
    await sCompStrategy.connect(deployer).setTokenSwapPathV2(tokenAddress.cvx, tokenAddress.frax, [tokenAddress.cvx, tokenAddress.weth, tokenAddress.frax],0);
