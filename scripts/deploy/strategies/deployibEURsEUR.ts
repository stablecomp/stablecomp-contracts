...
const info = require('../../../strategyInfo/infoPool/ibEURsEUR.json');
...
async function setupContract(): Promise<void> {
    ...
    await sCompStrategy.connect(deployer).setTokenSwapPathV2(tokenAddress.crv, tokenAddress.ibEur, [tokenAddress.crv, tokenAddress.weth, tokenAddress.ibEur],0);
    await sCompStrategy.connect(deployer).setTokenSwapPathV2(tokenAddress.cvx, tokenAddress.ibEur, [tokenAddress.cvx, tokenAddress.weth, tokenAddress.ibEur],0);
