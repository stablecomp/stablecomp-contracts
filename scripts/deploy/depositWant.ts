...
async function deposit(): Promise<void> {
  ...
  let balance = await wantContract.balanceOf(deployer.address)

  let txApprove = await wantContract.connect(deployer).approve(sCompVault.address, ethers.constants.MaxUint256)
  await txApprove.wait();

  console.log("Balance of want: ", ethers.utils.formatEther(balance))
