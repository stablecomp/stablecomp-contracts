...
main()
    .then(async () => {
        await deployScompTask.deployFeeDistribution(mainnetAddress.sCompTokenContract.address, mainnetAddress.veScompContract.address, deployer.address, deployer.address);
        process.exit(0)
    })
    .catch((error: Error) => {
        console.error(error);
