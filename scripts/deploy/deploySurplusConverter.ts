...
main()
    .then(async () => {
        await deployScompTask.deploySurplusConverter(mainnetAddress.feeDistributionContract.address, tokenInfo.weth.address, deployer.address, deployer.address, [deployer.address, deployer.address]);
        process.exit(0)
    })
    .catch((error: Error) => {
        console.error(error);
