...
main()
    .then(async () => {
        ...
        // change fee
        await strategyTask.proposeChangeFeeStrategy(mainnetAddress.sCompTimelockController.address, strategyInfo.sCompStrategy.address, newFeeGovernance, minDelay);
        process.exit(0)
    })
    .catch((error: Error) => {
        console.error(error);
