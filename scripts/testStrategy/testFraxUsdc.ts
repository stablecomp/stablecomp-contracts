...
async function setupContract(): Promise<void> {
    ...
    // set tokenSwapPath
    await sCompStrategy.connect(governance).setTokenSwapPathV3(crvAddress, tokenAddress.usdc, [crvAddress, tokenAddress.usdc], [10000], 1);
    await sCompStrategy.connect(governance).setTokenSwapPathV3(cvxAddress, tokenAddress.usdc, [cvxAddress, tokenAddress.usdc], [10000], 1);
...
async function checkBalance(): Promise<void> {
    ...
    if (lastBalanceOfGovernance != 0) {
        let diffBalance = ethers.utils.formatUnits(balanceGovernance.sub(lastBalanceOfGovernance), 6)
    ...
    if (lastBalanceOfConverter != 0) {
        let diffBalance = ethers.utils.formatUnits(balanceConverter.sub(lastBalanceOfConverter), 6)
